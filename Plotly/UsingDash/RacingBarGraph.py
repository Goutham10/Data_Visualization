...
df = pd.read_csv('/home/vy/Downloads/suicide-rate-1990-2017.csv')
...
# print(n_frame)
# print(n_frame['one']['suicide rate (deaths per 100,000)'])
# print(n_frame['one']['country'])
fig = go.Figure(
    data = [
        go.Bar(
    ...
    layout = go.Layout(
        xaxis = dict(range = [0,60],
                     autorange = False,
                     title = dict(text = 'suicide rate (deaths per 100,000)'),
                    #  font = dict(size = 18)
                ),
        ...
        title = dict(text='Suicide Rates per Country: 1990',
